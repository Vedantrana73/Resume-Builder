...
function Resume1() {
  const resumeForm = {
    ...
    careerObjective: "Hello, I am a graphic designer with 5 years of experience in the field. I have worked with various clients and have a strong portfolio to showcase my skills. I am looking for a challenging role in a reputed organization where I can utilize my skills and grow professionally.",
    education: [
      {
  ...
  return (
    <>
      <div className='no-print'>
        ...
        <Padding />
        <div className='p-5'>
          <button className='px-4 py-3 bg-green-500 rounded-lg' onClick={handlePrint}>
            Print
          </button>
        </div>
      </div>
              ...
              <ul className='text-sm'>
                {resumeForm.languages.map((language) => (
                  <li key={language.name}>
                    ...
                    <div>
                      <progress value={language.familiarity} max={100} className='h-0.5'></progress>
                    </div>
                  </li>
                ))}
              </ul>
            ...
            <ul className='text-xs py-1.5'>
              {resumeForm.education.map((education) => (
                <li key={education.degree}>
                  ...
                  <div className='text-base'>{education.startYear} - {education.endYear}</div>
                </li>
              ))}
            </ul>
